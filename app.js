    ...
    if(a_player.x < this.x + this.w &&
       ...
       a_player.h + a_player.y > this.y) {
        ...
        if(currentGame.lives > 0){
            this.displayText(allTexts, texts[i], a_player.x, a_player.y, 0.7, 'red');
        }
        a_player.reset();
        ...
        if(currentGame.lives <= 0){
            ...
            i = Math.floor(Math.random() * gameover.length);
...
// create row arrays
// sort each item of allItems array into row arrays
// set each item row to the number of array.
// set each item col to a column from zero to five.
// iterate through the row arrays
// set condition, if any items in the array share the same col set visibility to false.    
